...
SELECT ss.snap_id,
  ...
  round((CCWAIT_DELTA			   /DECODE(NVL(executions_delta,0),0,1,executions_delta)),0) conc_wait,
  round((CLWAIT_DELTA			   /DECODE(NVL(executions_delta,0),0,1,executions_delta)),0) clus_wait,
  ...
  round((iowait_delta /DECODE(NVL(executions_delta,0),0,1,executions_delta)),0) iowait,
