-------------------------------------------------------------------------------------------------------
--
-- File name:   build_bind_vars_plsql.sql
--
-- Purpose:     Build SQL*Plus test script using pl/sql for dates 
--
...
dbms_output.put_line('isdigit: '||:isdigit);
end;
/
select case when :bind_count > 0 then 'declare' else '-- No Bind Variables' end txt from dual;
--
...
--
select case when :bind_count > 0 then 'begin' else null end txt from dual;
select
case when :bind_count > 0 then
   case :isdigits when 1 then replace(bind_name,':',':N') else replace(bind_name,':','') end ||
   ' := ' ||
...
select case :isdigits when 1 then replace(sql_fulltext,':',':N') else replace(sql_fulltext,':','') end ||';' sql_fulltext
from v$sqlarea
where sql_id = '&&sql_id');
