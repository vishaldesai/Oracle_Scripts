...
col priops 	format 999990.00 	head 'PhysReadIOPs'
col priopso format 999990.00 	head 'PhysReadIOPsOpt'
col riopshr format 999.90 head "%Read|IOPsOpt"
col wiopshr format 999.90 head "%Write|IOPsOpt"
...
WITH
  ...
  (
    ...
    (sum(value) for (stat_name) in
        (
         'cell flash cache read hits' as fcrh, 
         'physical read total bytes' as prtb,
				 'physical read total bytes optimized' as prtbo,
				 'physical read total IO requests' as priops,
				 --total IO requests IOPS from backups and other utilities
				 --'physical read IO requests' as priops,
				 'physical read requests optimized' as priopso,
				 'physical write total IO requests' as pwiops,
				 --'physical write IO requests' as pwiops,
				 'physical write requests optimized' as pwiopso))
  )
...
--     ,round(priopso)          priopso
     ,100*(priopso/priops)     riopshr
     ,100*(pwiopso/pwiops)     wiopshr
from (
SELECT
  b2.instance_number,
  ...
  (b2.pwiops - b1.pwiops)/60/inter.inter_val    pwiops,
  (b2.pwiopso - b1.pwiopso)/60/inter.inter_val  pwiopso
FROM
  base_line b1,
  base_line b2,
  inter
