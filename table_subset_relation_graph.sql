...
var v_tablist VARCHAR2(2000);

declare
cursor c1 is
SELECT
       distinct replace(relationship,'->',''',''') rel
FROM
       (
              SELECT /*+ parallel(4) */
...
column v_tab new_val v_tab
select :v_tablist v_tab from dual;
...
select '{ from: "' || parent_table_name || '"' || ', to: "' || table_name || '"' || ', text: "0..N", toText: "1" },'
  ...
  SELECT DISTINCT
	...
	where parent_table_name is not null
	  and ( parent_Table_name in (&v_tab));
...
host start chrome C:\Users\U267399\Desktop\Tools\scripts\logs\er_subset.html