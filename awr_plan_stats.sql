...
col avg_etime for 999,999,999.999
...
execs,
--avg_etime/(px_servers/(execs*2)) as avg_etime,
avg_etime/((execs)) as avg_etime,
px_servers/execs avg_px_servers,
avg_cpu_time,
avg_lio,
avg_pio,
avg_rows from(
...
sum(ROWS_PROCESSED_DELTA)/sum(execs) avg_rows,
decode(sum(PX_SERVERS_EXECS_DELTA),0,2,sum(PX_SERVERS_EXECS_DELTA)) px_servers
from (
