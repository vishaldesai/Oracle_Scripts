...
accept event_name       prompt 'Enter event_name			: '
...
WITH
  ...
  (
	...
	AND stm.instance_number = snp.instance_number
	AND stm.snap_id         = snp.snap_id
...
SELECT
  ...
  --round((b2.time_waited_micro - b1.time_waited_micro)/(b2.total_waits - b1.total_waits)/1000,1) avg_time_ms
  round((b2.time_waited_micro - b1.time_waited_micro)/decode((b2.total_waits - b1.total_waits),0,1,(b2.total_waits - b1.total_waits))/1000,1) avg_time_ms,
  round((b2.time_waited_micro  - b1.time_waited_micro)*100/(b2.dbt - b1.dbt),2) pdbt
FROM
  base_line b1,
  base_line b2,
  inter
