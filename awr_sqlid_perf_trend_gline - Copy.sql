...
accept sql_id           default "g8kjkdym15fdy"     prompt 'Enter stats name			: '
accept days_history     default 30				    prompt 'Enter number of days			: '
accept interval_hours   default 24					prompt 'Enter interval in hours (60,120...)	: '
...
PRO
SPO awr_sqlid_perf_trend_gline.html;
PRO <html>
PRO <head>
...
PRO ['Date Column', 'CPU', { role: "style" } ,'IO' , { role: "style" } ,'CLUSTER', { role: "style" } ,'APPLICATION', { role: "style" } ,'CONCURRENCY', { role: "style" } ,'PLSQL', { role: "style" } ,'JAVA', { role: "style" } ]
/****************************************************************************************/
select 
	...
	',' || to_char(trunc(sysdate-&days_history+1)+trunc((cast(hs.end_interval_time as date)-(trunc(sysdate-&days_history+1)))*24/(&interval_hours))*(&interval_hours)/24,'MM') || /* month */
    ...
    ','||  round(sum(hss.apwait_delta)/1000000/decode(sum(hss.executions_delta),0,1,sum(hss.executions_delta)),3)          || ',"Black"' ||
...
--PRO width: 1300,
--PRO height: 900,
PRO chartArea: {width: '90%', height: '90%'},
PRO legend: { position: 'top', maxLines: 3 },
PRO bar: { groupWidth: 30 },
PRO colors: ['Green','Blue','Gray','Black','Brown','Purple','Pink'],
PRO isStacked: true
PRO }
PRO
...
host start chrome C:\Users\U267399\Desktop\Tools\scripts\awr_sqlid_perf_trend_gline.html