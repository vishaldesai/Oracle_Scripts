-- ash4.sql print info from from ash for 11.2, for RAC 4 nodes
-- Luca March 2012
...
from (select * from gv$active_session_history where inst_id=1 and rownum<100) 
...
select inst_id||'_'||session_id||' '||session_serial# sid_ser#,
       ...
       round(delta_read_io_bytes/delta_time,1)||', '||round(delta_write_io_bytes/delta_time,1) "R,W_MBPS",
       round(pga_allocated/1000000,1)||', '||round(temp_space_allocated/1000000,1) "PGA,TEMP_MB"
from (select * from gv$active_session_history where inst_id=2 and rownum<100) 
...
select inst_id||'_'||session_id||' '||session_serial# sid_ser#,
       ...
       round(delta_read_io_bytes/delta_time,1)||', '||round(delta_write_io_bytes/delta_time,1) "R,W_MBPS",
       round(pga_allocated/1000000,1)||', '||round(temp_space_allocated/1000000,1) "PGA,TEMP_MB"
from (select * from gv$active_session_history where inst_id=3 and rownum<100) 
...
select inst_id||'_'||session_id||' '||session_serial# sid_ser#,
       ...
       round(delta_read_io_bytes/delta_time,1)||', '||round(delta_write_io_bytes/delta_time,1) "R,W_MBPS",
       round(pga_allocated/1000000,1)||', '||round(temp_space_allocated/1000000,1) "PGA,TEMP_MB"
from (select * from gv$active_session_history where inst_id=4 and rownum<100) 
